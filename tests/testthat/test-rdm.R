...
test_that("rrdm behaves correctly under various conditions", {
  ...
  sim_data_t1 <- rrdm(n = 100, vzero = 0.8, vone = 0.6, bs = bs_val, bias = bias_val, ndt = ndt_val)
  ...
  # Case 2.1: vzero > vone
  sim_21 <- rrdm(n = n, vzero = 1.0, vone = 0.7, bs = bs_val, bias = bias_val, ndt = ndt_val)
  expect_gt(mean(sim_21$response == 0), 0.5, label = "vzero > vone: expect P(response=0) > 0.5")

  # Case 2.2: vone > vzero
  sim_22 <- rrdm(n = n, vzero = 0.6, vone = 0.9, bs = bs_val, bias = bias_val, ndt = ndt_val)
  ...
  sim_23 <- rrdm(n = n, vzero = 0.8, vone = 0, bs = bs_val, bias = bias_val, ndt = ndt_val)
...
test_that("drdm matches the simulated distribution and log=TRUE works", {
  set.seed(2025)
  pars <- list(vzero = 0.9, vone = 0.7, bs = 0.4, bias = 0.15, ndt = 0.2)

  dat <- do.call(rrdm, c(list(n = 20000), pars))
  ...
  y_th <- do.call(drdm, c(list(x = emp$x[keep]), pars))
...
test_that("drdm defective densities decompose the marginal and match simulation", {
  set.seed(11)
  pars <- list(vzero = 0.9, vone = 0.7, bs = 0.4, bias = 0.15, ndt = 0.2)
  xs <- c(0.25, 0.4, 0.8, 1.5, 3)

  marg <- do.call(drdm, c(list(x = xs), pars))
  d0 <- do.call(drdm, c(list(x = xs, response = 0), pars))
  d1 <- do.call(drdm, c(list(x = xs, response = 1), pars))
  ...
  i0 <- integrate(function(z) do.call(drdm, c(list(x = z, response = 0), pars)),
                  0, Inf, rel.tol = 1e-9, subdivisions = 2000)$value
  i1 <- integrate(function(z) do.call(drdm, c(list(x = z, response = 1), pars)),
                  0, Inf, rel.tol = 1e-9, subdivisions = 2000)$value
  expect_equal(i0 + i1, 1, tolerance = 1e-6)

  sim <- do.call(rrdm, c(list(n = 50000), pars))
  expect_equal(mean(sim$response == 0), i0, tolerance = 0.01)

  expect_error(do.call(drdm, c(list(x = 0.5, response = 2), pars)))
})


test_that("prdm agrees with integrating drdm and with simulation", {
  set.seed(99)
  pars <- list(vzero = 0.9, vone = 0.7, bs = 0.4, bias = 0.15, ndt = 0.2)
  sim <- do.call(rrdm, c(list(n = 50000), pars))

  for (q in c(0.3, 0.5, 1.0, 2.0)) {
    surv <- do.call(prdm, c(list(q = q, lower.tail = FALSE), pars))
    num <- integrate(function(z) do.call(drdm, c(list(x = z), pars)),
                     q, Inf, rel.tol = 1e-10, subdivisions = 2000)$value
    expect_equal(surv, num, tolerance = 1e-7,
                 label = sprintf("prdm survival vs integrated density at q=%.1f", q))
    expect_equal(surv, mean(sim$rt > q), tolerance = 0.02,
                 label = sprintf("prdm survival vs empirical at q=%.1f", q))
  }

  # CDF and survival are complementary, and log.p is consistent
  q <- c(0.3, 0.8, 2.0)
  lo <- do.call(prdm, c(list(q = q), pars))
  hi <- do.call(prdm, c(list(q = q, lower.tail = FALSE), pars))
  expect_equal(lo + hi, rep(1, length(q)), tolerance = 1e-12)
  expect_equal(do.call(prdm, c(list(q = q, log.p = TRUE), pars)), log(lo),
               tolerance = 1e-10)
})
...
test_that("the driftless Wald CDF is correct (regression: was off by a factor of 2)", {
  ...
  # unreachable from drdm() at the time, but the Stan version reaches it
...
test_that("the Wald density integrates to one across the parameter grid", {
  for (prm in list(c(3, 0.5, 0.2), c(1, 1, 0.5), c(0.5, 2, 0.1),
    ...
    expect_equal(tot, 1, tolerance = 1e-6,
                 label = sprintf("total mass at drift=%g, bs=%g, bias=%g", nu, k, A))
  }
})
...
test_that("drdm edge-cases x<=ndt give zero density", {
  ...
  dens <- drdm(xs, vzero = p$vzero, vone = p$vone, bs = p$bs, bias = p$bias, ndt = p$ndt)
  ...
  lg <- drdm(p$ndt + c(1e-8, 1e-6, 1e-4), vzero = p$vzero, vone = p$vone,
             bs = p$bs, bias = p$bias, ndt = p$ndt, log = TRUE)
...
rdm_lpdf_cached <- function() {
  if (is.null(.rdm_stan_cache$fun)) {
    .rdm_stan_cache$fun <- rdm_lpdf_expose()
  }
  .rdm_stan_cache$fun
}

test_that("Stan rdm_lpdf matches R drdm", {
  skip_on_cran()
  skip_if_not_installed("cmdstanr")

  rdm_lpdf_stan <- rdm_lpdf_cached()
  ...
  stan_ll <- mapply(
    rdm_lpdf_stan,
    grid$Y, grid$mu, grid$driftone, grid$sigmabias, grid$bs,
    grid$tau, minrt, grid$dec
  )
  r_ll <- mapply(
    function(Y, mu, d1, sb, bs, tau, dec) {
      drdm(x = Y, response = dec, vzero = mu, vone = d1, bs = bs,
           bias = sb, ndt = tau * minrt, log = TRUE)
    },
    grid$Y, grid$mu, grid$driftone, grid$sigmabias, grid$bs, grid$tau, grid$dec
  )
...
test_that("Stan rdm_lpdf is finite in the tails and for fast responses", {
  skip_on_cran()
  skip_if_not_installed("cmdstanr")
  ...
  for (prm in list(c(5, 10), c(8, 10), c(20, 6))) {
    ll <- rdm_lpdf_stan(prm[1], prm[2], prm[2], 0.2, 0.5, 0.5, 0.2, 0)
    expect_true(is.finite(ll),
                label = sprintf("finite at Y=%g, drift=%g", prm[1], prm[2]))
  }

  # Responses just above the non-decision time, where (bs - drift*t)/sqrt(t)
  # grows past 8 and Stan's own std_normal_lccdf would return -Inf.
  for (dy in c(1e-2, 1e-3, 1e-4)) {
    ll <- rdm_lpdf_stan(0.1 + dy, 2, 1.5, 0.3, 1.0, 0.5, 0.2, 0)
    expect_true(is.finite(ll),
                label = sprintf("finite at %g above the ndt", dy))
  }

  # Y at or below the non-decision time is impossible
  expect_equal(rdm_lpdf_stan(0.05, 2, 1, 0.3, 0.5, 0.9, 0.2, 0), -Inf)
  # Invalid inputs
  expect_equal(rdm_lpdf_stan(0.5, 2, 1, 0.3, 0.5, 0.5, 0.2, 2), -Inf)
  expect_equal(rdm_lpdf_stan(0.5, 2, 1, 0, 0.5, 0.5, 0.2, 0), -Inf)
  expect_equal(rdm_lpdf_stan(0.5, 2, 1, 0.3, 0, 0.5, 0.2, 0), -Inf)
})


# End-to-end parameter recovery. Left commented out (as in test-lba.R) because
# it takes several minutes, but run it before a release.
#
# test_that("rdm model recovers its parameters with brms", {
#   skip_on_cran()
#   skip_if_not_installed("cmdstanr")
#
#   set.seed(2024)
#   true <- list(vzero = 2.5, vone = 1.6, bs = 0.8, bias = 0.3,
#                tau = 0.6, minrt = 0.25)
#   df <- rrdm(n = 4000, vzero = true$vzero, vone = true$vone, bs = true$bs,
...
#   )
#
#   # `sigmabias` and `bs` trade off along a very flat ridge (see ?rdm), so a
...
#
#   fit <- brms::brm(f, data = df, stanvars = rdm_stanvars(), prior = pr,
...
#   expect_equal(sp(po[["b_bs_Intercept"]]) + sp(po[["b_sigmabias_Intercept"]]),
#                true$bs + true$bias, tolerance = 0.15)
#
...
test_that("rdm() family is registered consistently with the Stan signature", {
  fam <- rdm()
  expect_equal(fam$name, "rdm")
  # Order matters: brms passes the dpars to rdm_lpdf in exactly this order.
  expect_equal(fam$dpars, c("mu", "driftone", "sigmabias", "bs", "tau", "minrt"))
  # brms stores the bounds as character
  expect_equal(fam$ub[["tau"]], "1")
  expect_true(all(unlist(fam$lb) == "0"))
  expect_true(all(is.na(unlist(fam$ub)[c("mu", "driftone", "sigmabias", "bs")])))
