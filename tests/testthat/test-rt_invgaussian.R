...
test_that("rrt_invgaussian matches theoretical moments", {
  n_samples <- 15000
  set.seed(123)
  ...
  for (drift in drift_values) {
    for (bs in bs_values) {
      for (ndt in ndt_values) { # Keep ndt
        ...
        rts <- rrt_invgaussian(n_samples, drift = drift, bs = bs, ndt = ndt) # Keep ndt

        if(any(is.na(rts))) {
            warning("NA values generated by rrt_invgaussian for ", label)
            next
        }
...
test_that("drt_invgaussian integrates correctly", {
    ...
    for (bs in bs_values) {
      for (ndt in ndt_values) {
        ...
        integrand <- function(x_int) {
          drt_invgaussian(x_int, drift = drift, bs = bs, ndt = ndt)
        }
        x_sample_range <- rrt_invgaussian(n_samples_for_range, drift = drift, bs = bs, ndt = ndt)
...
test_that("prt_invgaussian matches rrt_invgaussian and has correct properties", {
  n_samples <- 20000 # More samples for better ECDF accuracy
  set.seed(789)
  tol_cdf <- 0.05 # Tolerance for comparing prt_invgaussian to ECDF
  ...
  for (drift in drift_values) {
    for (bs in bs_values) {
      for (ndt in ndt_values) {
        ...
        # Generate samples
        rts <- rrt_invgaussian(n_samples, drift = drift, bs = bs, ndt = ndt)
        if(any(is.na(rts))) {
            warning("NA values generated by rrt_invgaussian for ", label)
            next
        }
        ...
        # 1. Compare prt_invgaussian with empirical CDF
        p_values <- prt_invgaussian(test_quantiles, drift = drift, bs = bs, ndt = ndt)
        ecdf_values <- ecdf_rts(test_quantiles)
        expect_equal(p_values, ecdf_values, tolerance = tol_cdf,
                     label = paste(label, "- CDF: prt_invgaussian vs Empirical"))

        # 2. Check boundary conditions
        expect_equal(prt_invgaussian(ndt, drift = drift, bs = bs, ndt = ndt), 0,
                     label = paste(label, "- CDF at ndt is 0"))
        expect_equal(prt_invgaussian(ndt - 1e-6, drift = drift, bs = bs, ndt = ndt), 0,
                     label = paste(label, "- CDF below ndt is 0"))
        expect_equal(prt_invgaussian(Inf, drift = drift, bs = bs, ndt = ndt), 1,
                     label = paste(label, "- CDF at Inf is 1"))

        # 3. Check lower.tail argument
        p_upper <- prt_invgaussian(test_quantiles, drift = drift, bs = bs, ndt = ndt, lower.tail = FALSE)
        expect_equal(p_upper, 1 - p_values, tolerance = 1e-7,
                     label = paste(label, "- CDF lower.tail=FALSE"))

        # 4. Check log.p argument
        p_log <- prt_invgaussian(test_quantiles, drift = drift, bs = bs, ndt = ndt, log.p = TRUE)
