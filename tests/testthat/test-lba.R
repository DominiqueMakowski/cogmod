          ...
          for (sigmabias in sigmabias_vals) {
            for (bs in bs_vals) {
              for (ndt in ndt_vals) {
                ...
                param_label <- sprintf(
                  "driftzero=%.1f, driftone=%.1f, sigmazero=%.1f, sigmaone=%.1f, sigmabias=%.1f, bs=%.1f, ndt=%.1f",
                  driftzero,
                  driftone,
                  sigmazero,
                  sigmaone,
          ...
          for (sigmabias in sigmabias_vals) {
            for (bs in bs_vals) {
              for (ndt in ndt_vals) {
                ...
                label <- sprintf(
                  "params(driftzero=%.1f, driftone=%.1f, sigmazero=%.1f, sigmaone=%.1f, sigmabias=%.1f, bs=%.1f, ndt=%.1f)",
                  driftzero,
                  driftone,
                  sigmazero,
                  sigmaone,
              ...
              for (tau in tau_values) {
                for (dec in dec_values) {
                  ...
                  for (Y in Y_values) {
                    ...
                    label <- sprintf(
                      "Y=%.2f, driftzero=%.1f, driftone=%.1f, sz0=%.1f, sz1=%.1f, sb=%.1f, bs=%.1f, tau=%.2f, dec=%d",
                      Y,
                      driftzero,
                      driftone,
                      sigmazero,
