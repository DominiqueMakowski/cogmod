...
test_that("rcogmod_lba2 generates consistent data across various parameter settings", {
  skip_on_cran()
...
test_that("rcogmod_lba2 is vectorized over its parameters", {
  # `posterior_predict_cogmod_lba2()` passes one parameter value per posterior draw, so
  # `rcogmod_lba2()` must honour them trial by trial. Two regressions are guarded here:
  ...
  d <- rcogmod_lba2(
    ...
    sigmazero = 0.3, sigmaone = 0.3, sigmabias = 0.5, boundary = 0.5, ndt = 0.2
  )
  ...
  d2 <- rcogmod_lba2(
    ...
    sigmabias = 0.5, boundary = 0.5, ndt = rep(c(0.1, 1.0), each = n / 2)
  )
  expect_lt(min(d2$rt[half]), 0.5)
  expect_gt(min(d2$rt[-half]), 1.0)

  # Validation still fires when only one element is invalid.
  expect_error(rcogmod_lba2(n = 3, sigmazero = c(1, 1, -1)))
  expect_error(rcogmod_lba2(n = 3, boundary = c(0.5, -1, 0.5)))
  expect_error(rcogmod_lba2(n = 3, ndt = c(0.1, 0.1, -0.1)))
})


test_that("dcogmod_lba2 integrates correctly and matches rcogmod_lba2 empirical probabilities", {
  skip_on_cran()
...
#   true_boundary <- 0.3             # threshold offset (b = sigmabias + boundary)
#   true_tau <- 0.8            # non-decision time
#   true_minrt <- 0.2
#
#   # Generate data using our simulation function rcogmod_lba2()
#   df <- rcogmod_lba2(n,
...
#   # Parameters: mu, driftone, sigmazero, sigmaone, sigmabias, boundary, and ndt.
#   f <- brms::bf(
...
#   )
#
#   # Fit the model in brms with our custom family cogmod_lba2() and stanvars cogmod_lba2_stanvars()
#   fit <- brms::brm(
#     formula = f,
#     data = df,
#     family = cogmod_lba2(),            # our custom family
#     stanvars = cogmod_lba2_stanvars(), # our custom Stan functions (that include cogmod_lba2_lpdf)
#     init = 0,
...
#
#   # Standard deviations and sigmabias/boundary use softplus links: transform via log1p(exp(.))
...
#   est_boundary <- log1p(exp(means[["b_boundary_Intercept"]]))
#   expect_equal(est_boundary, true_boundary, tolerance = 0.15,
...
#
#
test_that("Stan cogmod_lba2_lpdf matches R dcogmod_lba2 function", {
  skip_on_cran()
  skip_if_not_installed("cmdstanr")

  # Expose the Stan lpdf function from our custom Stan functions.
  lba_lpdf_stan <- cogmod_lba2_lpdf_expose()
            ...
            for (boundary in boundary_values) {
              for (tau in tau_values) {
                for (dec in dec_values) {
                  ...
                  if (
                    (dec == 0 && driftzero <= 0) || (dec == 1 && driftone <= 0)
                  ) {
                    next
                  }
                  ...
                  for (Y in Y_values) {
                    ...
                    label <- sprintf(
                      "Y=%.2f, driftzero=%.1f, driftone=%.1f, sz0=%.1f, sz1=%.1f, sb=%.1f, boundary=%.1f, tau=%.2f, dec=%d",
                      Y,
                      driftzero,
                      driftone,
                      sigmazero,
                  ...
                  if (Y_below > 0) {
                    ...
                    expect_equal(
                      ...
                      r_lpdf_below,
                      label = sprintf(
                        "Below ndt mismatch: Y=%.2f, ndt=%.2f",
                        Y_below,
                        ndt
                      )
                    )
                    expect_equal(
                      ...
                      -Inf,
                      label = sprintf(
                        "Should be -Inf when Y < ndt: Y=%.2f, ndt=%.2f",
                        Y_below,
                        ndt
                      )
                    )
  ...
  expect_equal(
    ...
    -Inf,
    label = "Stan invalid sigmazero=0"
  )
  expect_equal(
    lba_lpdf_stan(
      Y = 0.5,
