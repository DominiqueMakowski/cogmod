  ...
  for (drift in drift_values) {
    for (bs in bs_values) {
      for (ndt in ndt_values) { # Keep ndt
        label <- sprintf("params(drift=%.1f, bs=%.1f, ndt=%.1f)", drift, bs, ndt) # Keep ndt
        rts <- rshifted_wald(n_samples, drift = drift, bs = bs, ndt = ndt) # Keep ndt
        ...
        theo_mean <- bs / drift + ndt # Keep ndt
        theo_var <- bs / (drift^3)
...
test_that("dshifted_wald integrates correctly", {
  ...
  bs_values <- c(0.2, 0.7, 1.5)
  ndt_values <- c(0.05, 0.2, 0.4)

  for (drift in drift_values) {
    for (bs in bs_values) {
      for (ndt in ndt_values) {
        label <- sprintf("params(drift=%.1f, bs=%.1f, ndt=%.1f)", drift, bs, ndt)
        integrand <- function(x_int) {
          dshifted_wald(x_int, drift = drift, bs = bs, ndt = ndt)
        }
        x_sample_range <- rshifted_wald(n_samples_for_range, drift = drift, bs = bs, ndt = ndt)
  ...
  for (drift in drift_values) {
    for (bs in bs_values) {
      for (ndt in ndt_values) {
        ...
        # Generate samples
        rts <- rshifted_wald(n_samples, drift = drift, bs = bs, ndt = ndt)
        if(any(is.na(rts))) {
            warning("NA values generated by rshifted_wald for ", label)
            next
        }
        ...
        p_values <- pshifted_wald(test_quantiles, drift = drift, bs = bs, ndt = ndt)
        ...
        expect_equal(pshifted_wald(ndt, drift = drift, bs = bs, ndt = ndt), 0,
                     label = paste(label, "- CDF at ndt is 0"))
        expect_equal(pshifted_wald(ndt - 1e-6, drift = drift, bs = bs, ndt = ndt), 0,
                     label = paste(label, "- CDF below ndt is 0"))
        expect_equal(pshifted_wald(Inf, drift = drift, bs = bs, ndt = ndt), 1,
                     label = paste(label, "- CDF at Inf is 1"))

        # 3. Check lower.tail argument
        p_upper <- pshifted_wald(test_quantiles, drift = drift, bs = bs, ndt = ndt, lower.tail = FALSE)
        expect_equal(p_upper, 1 - p_values, tolerance = 1e-7,
                     label = paste(label, "- CDF lower.tail=FALSE"))

        # 4. Check log.p argument
        p_log <- pshifted_wald(test_quantiles, drift = drift, bs = bs, ndt = ndt, log.p = TRUE)
...
test_that("Shifted Wald model can recover parameters with brms", {
  ...
  # Simulate data
  df <- data.frame(rt = rshifted_wald(n_obs, drift = true_mu, bs = true_bs, ndt = true_ndt))
  ...
  expect_equal(fixed_effects["bs_Intercept", "Estimate"], log(true_bs), tolerance = 0.5, label = "bs recovery")
...
test_that("Stan shifted_wald_lpdf matches R dshifted_wald function", {
      ...
      for (tau_val in tau_values) {
        for (minrt_val in minrt_values) {
          for (y_offset in y_offsets) {
            ...
            r_loglik <- dshifted_wald(y_val, drift = mu_val, bs = bs, ndt = ndt_val, log = TRUE)

            # Calculate Stan log-likelihood
            stan_loglik <- shifted_wald_lpdf(Y = y_val, mu = mu_val, bs = bs, tau = tau_val, minrt = minrt_val)
  ...
  # Case where Y < ndt
  y_below <- 0.15
  r_loglik_below <- dshifted_wald(y_below, drift = mu_val, bs = alpha_val, ndt = ndt_val, log = TRUE)
  stan_loglik_below <- shifted_wald_lpdf(Y = y_below, mu = mu_val, bs = alpha_val, tau = tau_val, minrt = minrt_val)
  ...
  y_equal <- ndt_val
  r_loglik_equal <- dshifted_wald(y_equal, drift = mu_val, bs = alpha_val, ndt = ndt_val, log = TRUE)
  stan_loglik_equal <- shifted_wald_lpdf(Y = y_equal, mu = mu_val, bs = alpha_val, tau = tau_val, minrt = minrt_val)
  ...
  # Invalid mu
  stan_loglik_bad_mu <- shifted_wald_lpdf(Y = y_val, mu = -1.0, bs = alpha_val, tau = tau_val, minrt = minrt_val)
  expect_equal(stan_loglik_bad_mu, -Inf, label = "Stan invalid mu")

  # Invalid bs
  stan_loglik_bad_alpha <- shifted_wald_lpdf(Y = y_val, mu = mu_val, bs = 0.0, tau = tau_val, minrt = minrt_val)
  expect_equal(stan_loglik_bad_alpha, -Inf, label = "Stan invalid bs")

  # Invalid tau (< 0)
  stan_loglik_bad_tau1 <- shifted_wald_lpdf(Y = y_val, mu = mu_val, bs = alpha_val, tau = -0.1, minrt = minrt_val)
  expect_equal(stan_loglik_bad_tau1, -Inf, label = "Stan invalid tau (<0)")

  # Invalid tau (> 1)
  stan_loglik_bad_tau2 <- shifted_wald_lpdf(Y = y_val, mu = mu_val, bs = alpha_val, tau = 1.1, minrt = minrt_val)
  ...
  r_loglik_tau0 <- dshifted_wald(y_val_tau0, drift = mu_val, bs = alpha_val, ndt = ndt_zero, log = TRUE)
  stan_loglik_tau0 <- shifted_wald_lpdf(Y = y_val_tau0, mu = mu_val, bs = alpha_val, tau = 0.0, minrt = minrt_val)
  ...
  r_loglik_tau1 <- dshifted_wald(y_val_tau1, drift = mu_val, bs = alpha_val, ndt = ndt_one, log = TRUE)
  stan_loglik_tau1 <- shifted_wald_lpdf(Y = y_val_tau1, mu = mu_val, bs = alpha_val, tau = 1.0, minrt = minrt_val)
