...
insight::get_predicted(m_lnr, data = df[1:3, ], iterations = 3, predict = "prediction") |> 
  as.data.frame()

pred <- estimate_prediction(m_lnr, data = df[1:3, ], iterations = 3, keep_iterations = TRUE) |>
  as.data.frame()
    reshape_iterations()

split(pred, pred[["iter_group"]])


pred |> 
  # datawizard::data_select(select = c("Component", "iter_value", "iter_group", "iter_index")) |> 
  # datawizard::data_to_wide(id_cols=c("iter_group"), values_from="iter_value", names_from="Component") |> 
  head()
  
dat <- pred[pred$iter_group == 1, ]
rt <- dat$rt
respoonse <- dat$response


.density_rt_response <- function(rt, response) {
  rt_choice0 <- rt[response == 0]
  rt_choice1 <- rt[response == 1]
  xaxis <- seq(0, max(rt_choice0, rt_choice1)* 1.1, length.out = 1000)
...
density_rt_response <- function(data, rt="rt", response="response", by=NULL) {
  if (is.null(by)) {
    out <- .density_rt_response(data[[rt]], data[[response]])
  } else {
    out <- sapply(split(data, data[[by]]), function(x) {
      .density_rt_response(x[[rt]], x[[response]])
    }, simplify = FALSE)
    |>
      do.call(rbind, .)
...
df |> 
  ggplot(aes(x=rt)) +
  geom_histogram(data=df[df$response == 0,], aes(y=after_stat(count)), fill="darkgreen", bins=100) +
  geom_histogram(data=df[df$response == 1,], aes(y=after_stat(-count)), fill="darkred", bins=100) +
  theme_minimal()
```
