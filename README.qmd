...
- [ ] **Log-Student-t** - `log(RT - ndt) ~ t(nu)`. A robust LogNormal: heavy tails
  ...
  `nu -> Inf`, so it varies kurtosis where `cogmod_loggamma()` varies skew.
...
- [ ] **Birnbaum-Saunders (fatigue life)** - a first-passage-time distribution
  ...
  `(1/a) * (sqrt(T/b) - sqrt(b/T))` is exactly standard normal. Corresponds to an equal mixture between an inverse Gaussian and a reciprocal inverse Gaussian distribution. Conceptualizes evidence as accumulating in discrete cycles and only ever in favour of it (i.e, towards the boundary). What's random is the size of each increment, not its sign. The distribution comes from applying the CLT to the sum of these positive increments across n cycles, then treating n as continuous to get a first-crossing-time approximation. It's one-directional accumulation in discrete chunks, with a Gaussian approximation standing in for the exact hitting-time calculation.
- [ ] **Ex-Wald** ([Schwarz, 2001](https://link.springer.com/article/10.3758/bf03195403)) - a Wald
...
- [ ] **LATER / recinormal** ([Carpenter & Williams, 1995](https://doi.org/10.1038/377059a0)) - `1 / (RT - ndt)` normally
  ...
  single-accumulator `cogmod_lba1()` as its start-point range goes to zero.
