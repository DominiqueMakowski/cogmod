...
- [ ] **Log-logistic** - `log(RT - ndt) ~ Logistic`. The motivation is the *hazard
  function*: Weibull and Gamma hazards are monotone, whereas the log-logistic
...
- [ ] **Shifted sinh-arcsinh (log-SHASH)** ([Jones & Pewsey,
  2009](https://doi.org/10.1093/biomet/asp053)) - location, scale, skewness and
  tail weight as four separate parameters. Where `rt_loggamma()` ties skew and
...
- [ ] **Birnbaum-Saunders (fatigue life)** - a first-passage-time distribution
  ...
  `(1/a) * (sqrt(T/b) - sqrt(b/T))` is exactly standard normal.
