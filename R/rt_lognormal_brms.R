...
#' or [rt_loggamma()], controlling whether `posterior_predict()` and `posterior_epred()` describe the
...
#'
#' @param object A `brmsfit` fitted with the [rt_lognormal()] or
#'   [rt_loggamma()] family.
#'
...
#' [rt_lognormal()] or [rt_loggamma()]. This is the mixture *responsibility*
...
p_outlier <- function(object, summary = TRUE) {
  ...
  if (is.null(fam)) fam <- "rt_lognormal"
  spec <- .rt_spec(fam)
  ...
  dec <- lapply(spec$dpars, function(d) as_mat(brms::get_dpar(prep, d)))
  names(dec) <- spec$dpars
