...
#' @param x,q Vector of quantiles (integer ratings between 1 and `k`).
...
rbetadiscrete <- function(n, mu = 0.5, phi = 3, k = 5) {
  ...
  if (any(k < 1 | k != floor(k))) stop("k must be a positive integer (>= 1).")

  # --- Vectorization ---
  n_out <- max(n, length(mu), length(phi), length(k))
  if (n_out > 1 || n > 1) {
...
#' @export
dbetadiscrete <- function(x, mu = 0.5, phi = 3, k = 5, log = FALSE) {
  # --- Input Validation ---
  if (any(mu <= 0 | mu >= 1)) stop("mu must be strictly between 0 and 1.")
  if (any(phi <= 0)) stop("phi must be positive.")
  if (any(k < 1 | k != floor(k))) stop("k must be a positive integer (>= 1).")
...
#' @rdname rbetadiscrete
#' @export
pbetadiscrete <- function(q, mu = 0.5, phi = 3, k = 5, lower.tail = TRUE, log.p = FALSE) {
  # --- Input Validation ---
  if (any(mu <= 0 | mu >= 1)) stop("mu must be strictly between 0 and 1.")
  if (any(phi <= 0)) stop("phi must be positive.")
  if (any(k < 1 | k != floor(k))) stop("k must be a positive integer (>= 1).")
  ...
  p <- ifelse(qq <= 0, 0, ifelse(qq >= k, 1, stats::pbeta(qq / k, alpha, beta)))
...
#' @rdname rbetadiscrete
#' @export
qbetadiscrete <- function(p, mu = 0.5, phi = 3, k = 5, lower.tail = TRUE, log.p = FALSE) {
  # --- Input Validation ---
  if (any(mu <= 0 | mu >= 1)) stop("mu must be strictly between 0 and 1.")
  if (any(phi <= 0)) stop("phi must be positive.")
  if (any(k < 1 | k != floor(k))) stop("k must be a positive integer (>= 1).")
