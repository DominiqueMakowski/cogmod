...
- sigmandt:
  ...
  as the data, with `ndt` its lower bound. Default `0`.
...
would surface as spurious effects on the other parameters. Expressing it
directly is what avoids that.

`sigmandt` is the between-trial *range* of the non-decision time (`st0`
in the usual notation), expressed directly in the same unit as the data,
with `ndt` the lower bound of the resulting Uniform.
