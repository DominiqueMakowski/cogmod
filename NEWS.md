...
* The performance article is reorganised from the suggestions with no downside
  ...
  **Warm starts**: reusing the adapted metric and step size that `brms` keeps
  in a fit's metadata to shorten the warmup of a refit, and a `cmdstanr`-level
  pipeline that initializes MCMC from Pathfinder draws and their unconstrained
  covariance, then wraps the result back into a `brmsfit`, with the reasons
  never to fix the metric to a variational approximation. The approximation
