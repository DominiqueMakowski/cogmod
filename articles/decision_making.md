...
are believed to act on. Its deficit lies on a different axis: with all
three variability parameters fixed to `0`, a DDM can only produce errors
that are *faster* than correct responses, and in these data they are
slightly slower. Sampling from the fitted models makes it concrete - the
DDM predicts errors 0.110 s faster than correct responses (95% CI
`[-0.135, -0.086]`) against an observed `+0.014` s, while the three
races, each carrying a separate drift for the error accumulator, all
cover the observed value.

**DDM-5** isolates exactly that axis: same model, same threshold
mechanism, only `sigmadrift` freed. It is worth 104 ELPD points, moves
the DDM from last to third, and halves the error-timing gap (to
`-0.058`, `[-0.101, -0.002]`) without closing it. That is the useful
lesson, and it is about diagnosis rather than about which family wins: a
poor fit had an identifiable cause, the cause pointed at one parameter,
and freeing it recovered most of the gap.
...
(purple) places its error density to the left of the observed errors,
the three races put theirs on top of it, and **DDM-5** (blue) sits
between the two. In the upper half all five are close - which is the
point worth taking away: a model’s problem can be invisible if one only
