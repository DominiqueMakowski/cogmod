...
cat(sprintf(
  "%s trials, %d participants (%s)\n",
  format(nrow(d), big.mark = ","), n_distinct(d$Participant),
  paste(
    sprintf(
      "%s = %d", levels(d$Diagnosis),
      tapply(
        d$Participant, d$Diagnosis,
...
cat(sprintf(
  "CONTROL %.1f ms, ADHD %.1f ms, t(%.0f) = %.2f, p = %.3f\n",
...
stats <- d |>
  ...
  summarise(
    mean = mean(RT), sd = sd(RT), cv = sd(RT) / mean(RT),
