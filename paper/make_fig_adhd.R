...
#
#   Rscript make_fig_adhd.R
#
# Panel A: per-participant switch costs, the conventional executive-control
#          index, which does not separate the groups.
# Panel B: the RT distributions that index is computed from.
# Panel C: what the shifted LogNormal separates, on the response scale.
...
pA <- ggplot(sc, aes(x = 1000 * cost, fill = Diagnosis)) +
  ...
  geom_vline(
    data = sc |> group_by(Diagnosis) |> summarise(m = 1000 * mean(cost)),
...
# `ndt` is on a `log` link and is in seconds. It was called `tau` and was a
# logit-scaled fraction of a `minrt` argument before the 0.2.1 reparameterization;
# both are gone, so this is exp() of the linear predictor and nothing else.
ndt_c <- exp(p$b_ndt_Intercept)
ndt_a <- exp(p$b_ndt_Intercept + p$b_ndt_DiagnosisADHD)
s_c <- sp(p$b_sigma_Intercept)
s_a <- sp(p$b_sigma_Intercept + p$b_sigma_DiagnosisADHD)
dec_c <- exp(p$b_Intercept + s_c^2 / 2)
dec_a <- exp(p$b_Intercept + p$b_DiagnosisADHD + s_a^2 / 2)
...
eff <- bind_rows(
  ...
  mutate(par = factor(par, levels = c(
...
ggsave("figures/fig_adhd.png", g, width = 6.4, height = 4.3, dpi = 300, bg = "white")
