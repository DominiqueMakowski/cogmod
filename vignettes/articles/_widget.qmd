...
two in step. Two assumptions choose between three families, and each of them
...
The two assumptions can make a fourth combination that is not a model, so
whichever of them the model on show cannot vary is greyed out and put back to
1 - see `constrain`.
...
measures whichever of the three the assumptions make it.
...
only fills it with two of everything. The plot geometry lives in `cfg` and
`frame` rather than in the call, because `driftArcs` has to measure the same
frame in pixels. -->
...
/* The palette, and the only place it is written down: `cfg` below reads these
   ...
   Four parameter colours, three model ones, and the two the annotations take -
...
/* The first rate is `drift` in the models that have one and `driftzero` in the
   ...
   it from. Both names are written in the markup and one of them is shown, so
   that the file says what the two are and nothing has to reach into the DOM to
   retype a label. `model` puts the class on. */
...
/* The assumptions bar. What it holds are not parameters of a model but
   ...
   colour. It is a row that wraps, because more assumptions are meant to end
   up in it. `--seg` is the colour the chosen segment of every toggle in the
   bar takes; it is declared here so that `model` can set it in one place, and
   given the Wald's blue so that the bar is never briefly colourless. */
...
`driftone` [second accumulator]{.cogmod-full}
...
```{ojs}
//| echo: false
// Each of these two is only asked about while the other one is at 1 - see
// `constrain`, which is what greys out whichever of them the model on show
// cannot vary, and puts it back to 1 so that a toggle nobody can turn is not
// left stating something that is not true.
...
cfg = {
  ...
  return {tmax: 1.2, dt: 0.005, npaths: 30, nrace: 15, ybot: -0.5, ytop: 1.4,
          ...
          mbottom: 16, blue: hex("blue"), red: hex("red"),
          azure: hex("azure"), orange: hex("orange"), green: hex("green"),
          teal: hex("teal"), purple: hex("purple"), grey: hex("grey"),
          black: hex("black")};
}
```
...
```{ojs}
//| echo: false
// The two assumptions, put together into the one thing the rest of the figure
// asks about. Two accumulators under a single boundary is a race rather than a
// drift: two Walds, each with its own rate, both climbing to the one boundary,
// the first to arrive taking the trial. Two boundaries is already the drift
// diffusion and has no second accumulator to give, so `constrain` holds
// `naccum` at 1 there - this still reads both, because the pass in which the
// second boundary arrives is one in which it has not been put back yet.
...
// Wald. Everything geometric reads this rather than the slider - the lines,
// the ticks, the paths' stopping points, the drift arrows' targets - while the
// drift diffusion's density, which takes the separation as such, reads
// `boundary`.
edge = nbounds === 2 ? boundary / 2 : boundary
```
...
model = {
  ...
  // of the toggles, and the drift diffusion is the only one it tells apart.
  const it = nbounds === 2
    ? {name: "Drift Diffusion Model (DDM)", colour: cfg.azure}
    : race ? {name: "Racing Diffusion Model (RDM)", colour: cfg.blue}
           : {name: "Wald Model", colour: cfg.blue};
  const find = (sel) => document.querySelector(sel);
  const title = find(".cogmod-title");
  if (title) title.textContent = it.name;
  // What the first rate is called, and what colour the second one's slider
  // is, both follow the race; see the stylesheet above for the two rules the
  // class switches.
  const layout = find(".cogmod-layout");
  if (layout) layout.classList.toggle("cogmod-racing", race);
  ...
  // and the second drift rate nothing to say about a single accumulator. The
  // assumptions themselves are handled the other way round - see `constrain`.
  const show = (sel, on) => {
    const el = find(sel);
    if (el) el.hidden = !on;
  };
  ...
  // No segment can name a model by its position once there are two toggles, so
  // the colour is handed to the whole bar and the checked segments read it off.
  const bar = find(".cogmod-assumptions");
...
```{ojs}
//| echo: false
// The two toggles can make four combinations and only three of them are
// models: a race between two accumulators has no second boundary to give, and
// a drift diffusion is already a process between two boundaries and has no
// second accumulator. So whichever toggle the model on show cannot vary is
// greyed out and taken out of reach - both of them only in the Wald, where
// either choice leads somewhere.
//
// It is also put back to 1. A toggle nobody can turn is still a toggle
// stating something, and leaving it holding a 2 that is not in effect would
// have the drift diffusion claim two accumulators - and would spring the race
// on a reader who only asked for one boundary back. Resetting it costs the
// value it was holding, which is the right trade: the reader leaves a model by
// the toggle they arrived on, and lands on the Wald, which is where both
// choices are open again.
//
...
// 1, so nothing is dispatched and nothing re-runs.
constrain = {
  const lock = (sel, shut) => {
    ...
    if (shut && radios.length && !radios[0].checked) {
      radios[0].checked = true;       // the first segment is the 1
      radios[0].dispatchEvent(new Event("input", {bubbles: true}));
    }
  };
  lock(".cogmod-assumption-bounds", naccum === 2);
...
dragHandles = function(plot) {
  ...
  const edgeAt = () => nbounds === 2 ? at("boundary") / 2 : at("boundary");
  ...
  const handle = {
    ...
    driftone: {
      ...
      away: (p, rect) => {
        ...
        const v = at("driftone");
        const tip = driftTip(at("ndt"), edgeAt(), 0, v);
        return toSegment(p,
          {x: X.to(at("ndt"), rect), y: Y.to(0, rect)},
          {x: X.to(tip, rect), y: Y.to(v * (tip - at("ndt")), rect)});
      },
...
// once per input rather than on every move: it names the five `viewof`
...
bubbles = {
  for (const form of [viewof drift, viewof driftone, viewof boundary,
                      viewof bias, viewof ndt]) {
...
boundarySpan = {
  ...
  const [lo, hi] = nbounds === 2 ? [-edge, edge] : [0, edge];
  const mid = (lo + hi) / 2;
...
```{ojs}
//| echo: false
// The mean drift paths: out of the start point at each accumulator's rate, as
// far as `driftTip` allows. The race's two leave the same corner for the same
...
// and the wedge between the arrows is the head start the faster one has.
driftPaths = {
  const one = (v, name, colour) => {
    const x2 = driftTip(ndt, edge, start, v);
    return {name: name, colour: colour, x1: ndt, y1: start, x2: x2,
            y2: start + v * (x2 - ndt)};
  };
...
density = {
  ...
  if (race) return [up, curve(second, edge, 1, cfg.red)];
  if (nbounds === 1) return [up];
  return [up, curve(second, -edge, -1, cfg.red)];
}
```
...
//
// Two rows per trial, drawn whether or not the race is on: the second half is
// the second accumulator's, and the two accumulators are independent, so it
// has to be a draw of its own rather than the first half reused. Drawing it
// unconditionally is what keeps the first half the same sample on either side
// of the accumulator toggle, so switching models re-runs these trials rather
// than replacing them.
noise = {
  ...
  const step = () => sd * rnorm();
  return Array.from({length: 2 * cfg.npaths},
...
// moves. They set off from the start point, and a trial ends at whichever
// boundary is touched first, carrying the height it ended at so that the dot
...
trials = {
  ...
  const hits = [];
  const slot = [];
  const trace = (id, colour, pts) => {
    rows.push({id: id, colour: colour, t: 0, x: start},
              {id: id, colour: colour, t: ndt, x: start});
  ...
  if (race) {
    ...
    for (let k = 0; k < cfg.nrace; k++) {
      const dw0 = noise[k], dw1 = noise[k + cfg.npaths];
      const one = [], two = [];
      ...
      trace(2 * k, cfg.green, one);
      trace(2 * k + 1, cfg.red, two);
      slot.push(k, k);
    }
