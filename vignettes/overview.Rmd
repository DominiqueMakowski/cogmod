...
`r*()` to simulate, `d*()` for the density, and sometimes `p*()`/`q*()` - which
depend only on **stats** and are useful on their own for simulation, prior
predictive work, and teaching.

```{r}
library(cogmod)

set.seed(3)
x <- rbetagate(5000, mu = 0.6, phi = 4, pex = 0.15, bex = 0.4)
...
m <- brm(
  bf(rating ~ condition + (1 | participant), phi ~ 1, pex ~ 1, bex ~ 1),
  data = df,
  family = betagate(),
...
The pattern is the same for every model: `family = <model>()` paired with
`stanvars = <model>_stanvars()`.
